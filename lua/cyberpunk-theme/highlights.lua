local primary_bg = C.bg_2

local highlights = {
  Bold = { style = "bold" },
  Boolean = { fg = C.vivid_sky_blue },
  Character = { fg = C.dp },
  ColorColumn = { bg = C.bg_2 },
  Comment = { fg = C.comment, style = "italic" },
  Conceal = { fg = C.fg },
  Conditional = { fg = C.vivid_sky_blue },
  Constant = { fg = C.vivid_sky_blue },
  CursorColumn = { bg = C.bg_2 },
  Cursor = { fg = C.cursor_fg, bg = C.cursor_bg },
  CursorIM = { fg = C.cursor_fg, bg = C.cursor_bg },
  CursorLine = { bg = C.bg_2 },
  CursorLineNr = { fg = C.vivid_sky_blue, style = "bold" },
  Debug = { fg = C.red },
  Define = { fg = C.purple_test_2 },
  Delimiter = { fg = C.comment },
  DiffAdd = { fg = C.bg_2, bg = C.sign_add },
  DiffChange = { fg = C.bg_2, bg = C.sign_change },
  DiffDelete = { fg = C.bg_2, bg = C.sign_delete },
  Directory = { fg = C.vivid_sky_blue },
  EndOfBuffer = { fg = C.bg_2 },
  Error = { fg = C.purple_test, bg = C.bg_2, style = "bold" },
  ErrorMsg = { fg = C.purple_test, bg = C.bg_2, style = "bold" },
  Exception = { fg = C.purple_test_2 },
  FloatBorder = { fg = C.purple_test_2, bg = primary_bg },
  Float = { fg = C.cc },
  FoldColumn = { fg = C.fg, bg = primary_bg },
  Folded = { fg = C.fg, bg = primary_bg },
  Function = { fg = C.aureolin}, -- change this it looks like shit. go with purple or somethign idk
  Identifier = { fg = C.cyan_test },
  Ignore = { fg = C.cyan_test, bg = primary_bg, style = "bold" },
  Include = { fg = C.purple_test_2 },
  IncSearch = { fg = C.search_orange, bg = C.light_gray },
  Italic = { style = "italic" },
  Keyword = { fg = C.vivid_sky_blue },
  Label = { fg = C.purple_test_2 },
  lCursor = { fg = C.cursor_fg, bg = C.cursor_bg },
  LineNr = { fg = C.vivid_sky_blue },
  Macro = { fg = C.purple_test_2 },
  MatchParenCur = { style = "underline" },
  MatchParen = { fg  = C.hint_blue, bg = primary_bg, style = "underline" },
  MatchWordCur = { style = "underline" },
  MatchWord = { style = "underline" },
  ModeMsg = { fg = C.blanc, bg = primary_bg },
  MoreMsg = { fg = C.dp },
  MsgArea = { fg = C.blanc, bg = Config.transparent_background and "NONE" or primary_bg },
  MsgSeperator = { fg = C.blanc, bg = primary_bg },
  NonText = { fg = primary_bg },
  Normal = { fg = C.blanc, bg = Config.transparent_background and "NONE" or primary_bg },
  NormalFloat = { bg = primary_bg },
  NormalNC = { fg = C.blanc, bg = Config.transparent_background and "NONE" or primary_bg },
  Number = { fg = C.cc },
  Operator = { fg = C.blanc },
  Pmenu = { fg = C.light_gray, bg = C.tree_gray },
  PmenuSbar = { bg = primary_bg },
  PmenuSel = { bg = C.ui2_blue },
  PmenuThumb = { bg = C.comment },
  PreCondit = { fg = C.purple_test_2 },
  PreProc = {fg = C.purple_test_2 },
  Question = { fg = C.dp },
  QuickFixLine = { bg = C.ui5_blue },
  Repeat = { fg = C.purple_test_2 },
  Search = { fg = C.light_gray, bg = C.search_blue },
  SignColumn = { bg = primary_bg },
  SpecialChar = { fg = C.blanc },
  SpecialComment = { fg = C.comment },
  Special = { fg = C.dp },
  SpecialKey = { fg = C.vivid_sky_blue, style = "broad" },
  SpellBad = { fg = C.red_new, style = "underline" },
  SpellCap = { fg = C.aureolin, style = "underline" },
  SpellLocal = { fg = C.comment, style = "underline" },
  SpellRare = { fg = C.comment, style = "underline" },
  Statement = { fg = C.purple_test_2 },
  StorageClass = { fg = C.vivid_sky_blue },
  String = { fg = C.green },
  Structure = { fg = C.vivid_sky_blue },
  Substitute = { fg = C.light_gray, bg = C.search_orange },
  TabLine = { fg = C.light_gray, bg = primary_bg },
  TabLineFill = { fg = primary_bg, bg = primary_bg },
  TabLineSel = {fg = C.blanc, bg = primary_bg },
  Tag = { fg = C.vivid_sky_blue },
  TermCursor = { fg = C.cursor_fg, bg = C.cursor_bg },
  TermCursorNC = { fg = C.cursor_fg, bg = C.cursor_bg },
  Title = { fg = C.vivid_sky_blue, style = "bold" },
  Todo = { fg = C.magenta, bg = primary_bg, style = "bold" },
  Typedef = { fg = C.vivid_sky_blue },
  Type = { fg = C.cyan_test },
  Underlined = { style = "underline" },
  Variable = { fg = C.cyan_test },
  VertSplit = { fg = C.purple_test_2, bg = primary_bg },
  Visual = { bg = C.ui_blue },
  VisualNOS = { bg = primary_bg },
  WarningMsg = { fg = C.purple_test, bg = primary_bg },
  Whitespace = { fg = C.bg_2 },
  WildMenu = { fg = C.blanc, bg = C.ui2_blue},
}
return highlights
